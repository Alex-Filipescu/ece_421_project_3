...
import TootOttoBoardGrid from '../components/TootOttoBoardGrid';
...
function TootOttoPage() {
  ...
  useEffect(() => {
    const sendGameChoice = async () => {
      try {
        await axios.post("http://localhost:8000/api/getGame", {text: "TootOtto"});
        await handleRefresh();
  ...
  const handleRefresh = async() => {
    ...
    try{
      ...
      console.log("refreshed TootOtto")
    }catch (error){
